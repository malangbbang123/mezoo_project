...
def process_file(file_path, idx):
    ...
    try:
        ...
        if id in data_list:
            ...
            # Process the data in beat segments
            ...
            # Calculate average of the segments
...
if __name__ == "__main__":
    ...
    mat_data = glob.glob(os.path.join(path, "**", "*.mat"), recursive=True)[:3]
    X = []  # Raw signal data
    Y = []  # Reverse labels
    Z = []  # Patient IDs
    max_len = 0
    ...
    Y = np.array(Y)
    Z = np.array(Z)
    np.savez(f"/Volumes/Seagate/hicardi/code_check/test_with_ids.npz", x=X, y=Y, z=Z)
